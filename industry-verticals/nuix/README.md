...
Nuix is a copy of the Forma Lux (retail) demo, created to showcase a modern shopping experience. It highlights key features such as intuitive navigation, multilingual support, and engaging content presentation. Configure `NEXT_PUBLIC_DEFAULT_SITE_NAME` to match your XM Cloud site (for example `forma-lux` if you reuse that content tree).
...
6. Provide Editing host name `nuix` as per xmcloud.build.json
...
#### By default, the Nuix app supports the following locales:
- en (English)
- fr-FR (French)
- es-ES (Spanish)
